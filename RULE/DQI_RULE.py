...
class DQI_RULE:
    ...
    def set_rule(self, set_rule):
        ...
        for regex_set in regex_set_rule:
            for regex_name in regex_set["regex_name"].split(","):    
                if regex_name not in self.config["REGEX"] and regex_name not in self.config["RANGE"]:
                    raise RuleException(
                        "not exist REGEX Key : {}".format(regex_name)
                    )
                else:
                    if regex_set["name"] in self.config["REGEX_SET"]:
                        regex_list = self.config["REGEX_SET"][regex_set["name"]].split(",")
                        if regex_name in regex_list:
                            raise RuleException(
                        ...
                        self.config["REGEX_SET"][regex_set["name"]] = regex_name
        # BIN_SET
        for bin_regex_set in bin_regex_set_rule:
            bin_list = []
            if len(self.config["BIN_SET"]["BIN"]) != 0:
                ...
                if bin_regex_set["set_name"] in bin_list:
                    ...
                    if len(bin_list) == 0:
                        if self.mode == "DB":
                            ...
                            self.config["BIN_SET"]["BIN"] = bin_regex_set["set_name"]
                    else:
                        # update
        ...
        for unique_regex_set in unique_regex_set_rule:
            unique_list = []
            if len(self.config["UNIQUE_SET"]["UNIQUE"]) != 0:
            ...
            if unique_regex_set["set_name"] not in self.config["REGEX_SET"]:
                ...
                if unique_regex_set["set_name"] in unique_list:
                    ...
                    if len(unique_list) == 0:
                        ...
                        self.config["UNIQUE_SET"]["UNIQUE"] = unique_regex_set["set_name"]
                    else:
                        # update
        ...
        for regex_set in regex_set_rule:            
            if regex_set["name"] not in self.config["REGEX_SET"]:
                ...
                if regex_set["name"] in bin_list or regex_set["name"] in unique_list:
                    ...
                    for regex_name in regex_set["regex_name"].split(","):
                        if regex_name not in regex_list:
                            raise RuleException(
